...
const EditProject: React.FC = () => {
  ...
  // Convert object to DynamicSection array
  const objectToSections = (obj: Record<string, any>): DynamicSection[] => {
    const sections = Object.entries(obj).map(([key, value]) => ({
      key,
      value: String(value)
    }));
    return sections.length > 0 ? sections : [{ key: '', value: '' }];
  };
  ...
  useEffect(() => {
    if (project) {
      setFormData({
        ...
        sold_units: String(project.sold_units),
        reserved_units: String(project.reserved_units)
      });

      setPricingDetails(objectToSections(project.pricing_details));
      setQuickInfo(objectToSections(project.quick_info));
      setAmenities(objectToSections(project.amenities));
    }
  }, [project]);
